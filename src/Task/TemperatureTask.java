...
public class TemperatureTask implements Runnable {
    ...
    public TemperatureTask(int TaskID, String TaskName, UserList User, RoomList Room, SensorList SmokeSensor, boolean isDisabled, boolean repeatDaily, int AlarmDuration, int AlarmInterval,
                    ...
                    switch (Device.getKey().getDeviceName()) {
                        ...
                        case "Curtains":
                            try (Statement Statement = DB.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
                                ...
                                Logger.getLogger(TemperatureTask.class.getName()).log(Level.SEVERE, null, ex);
                            }
                            break;
                        case "Alarm":
